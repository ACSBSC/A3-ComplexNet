...
def plot(cluster_nodes, G, path, algorithm):  
    if path != 'airports_UW':
        print("Plottin Network "+path+" with algorithm "+algorithm)
        G = nx.Graph(G)
...
#El siguiente código es una copia del anterior pero para hacer pruebas    
'''def plot(cluster_nodes, G, path, algorithm):  
    ...
    if path != 'airports_UW':
        pos = nx.spring_layout(G)                   #Esta es a línea que trae problemas
    else:
